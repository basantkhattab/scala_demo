...
val baseLightPoint = Vector3D(RADIUS*cos(80.toRadians), RADIUS*cos(120.toRadians), RADIUS*sin(80.toRadians)).normalize
val axis = Vector3D(RADIUS*cos(angleFromX)*cos(angleFromZ), RADIUS*sin(angleFromX)*cos(angleFromZ), RADIUS*sin(angleFromZ)).normalize
...
class ParasDemo2 extends Effect(600, 600, "ParasDemo2"):
  ...
  def rotateColorVector(angle: Double) =
    ...
    val xNew = colorVector.x*(q0*q0+q1*q1-q2*q2-q3*q3) + colorVector.y*(2*(q1*q2-q0*q3)) + colorVector.z*(2*(q1*q3+q0*q2))
    val yNew = colorVector.x*(2*(q2*q1+q0*q3)) + colorVector.y*(q0*q0-q1*q1+q2*q2-q3*q3) + colorVector.z*(2*(q2*q3-q0*q1))
    val zNew = colorVector.x*(2*(q3*q1-q0*q2)) + colorVector.y*(2*(q3*q2+q0*q1)) + colorVector.z*(q0*q0-q1*q1-q2*q2+q3*q3)
    /* ei toimi
  ...
  def decideColor(x: Int, y: Int, z: Double): Boolean =
    ...
    val colorPerp = Vector3D(colorVector.y*axis.z-axis.y*colorVector.z, axis.x*colorVector.z-colorVector.x*axis.z, colorVector.x*axis.y-axis.x*colorVector.y).normalize
